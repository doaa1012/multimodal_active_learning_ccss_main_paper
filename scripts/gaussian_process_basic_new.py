...
class GPBasic:
    ...
    def select_next_point(self, X_candidates, strategy="EI", kappa=2.0, xi=0.01):

        mu, var = self.model.predict(X_candidates)
        mu = mu.reshape(-1)
        var = var.reshape(-1)
        ...
        y_best = np.max(self.model.Y)

        if strategy == "EI":
            improvement = mu - y_best - xi
            Z = improvement / sigma
            acq = improvement * norm.cdf(Z) + sigma * norm.pdf(Z)
            acq[sigma < 1e-9] = 0.0
            ...
            raise ValueError("Strategy must be 'EI', 'PI', 'UCB', or 'VAR'")

        best_idx = int(np.argmax(acq))
        best_x = X_candidates[best_idx].reshape(1, -1)

        return best_x, best_idx, acq
